...
class LoggerFactory:
    ...
    def _load_config(self) -> Dict:
        ...
        config_locations = [
            ...
            Path.home()
            / ".cartai/config/logging_config.yaml",  # 4. User's home directory
            Path(__file__).parent
            / "config/logging_config.yaml",  # 5. Package directory
        ]
        ...
        return {
            "version": 1,
            "formatters": {"default": {"format": DEFAULT_LOG_FORMAT}},
            "handlers": {
                "console": {
                    "class": "StreamHandler",
                    "formatter": "default",
        ...
        for fmt_name, fmt_config in env_config.get("formatters", {}).items():
            ...
            if "class" in fmt_config:
                try:
                    ...
                    print(
                        f"Warning: Failed to import formatter class for {fmt_name}, using default"
                    )

            if formatter_class:
                formatters[fmt_name] = formatter_class(
                    fmt_config.get("format", DEFAULT_LOG_FORMAT)
                )
            else:
                formatters[fmt_name] = logging.Formatter(
                    fmt_config.get("format", DEFAULT_LOG_FORMAT)
                )
                ...
                for key, value in handler_config.items():
                    if key not in ["class", "formatter", "level"]:
                        if (
                            ...
                            if value is None:
                                print(
                                    f"Warning: Environment variable {env_var} not set for handler {handler_name}"
                                )
                                continue
