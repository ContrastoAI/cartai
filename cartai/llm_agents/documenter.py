...
from pydantic import BaseModel, Field, DirectoryPath, SecretStr
...
class AIDocumenter(BaseModel):
    ...
    """
    model: str | None = Field(default="gpt-4o-mini", description="The LLM model to use for generation")
    api_key: SecretStr | None = Field(default=None, description="The API key to use for the LLM provider")
    ...
    def generate(
        ...
        api_key = self.api_key.get_secret_value() if self.api_key else os.getenv("OPENAI_API_KEY")
        if not api_key:
            raise ValueError(
